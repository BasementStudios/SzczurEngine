...
{
    LOG_INFO("assets/armatures/" + name + "/skeleton.json");
	auto skeletonData = factory.loadDragonBonesData("assets/armatures/" + name + "/skeleton.json");

    LOG_INFO("assets/armatures/" + name + "/texture.png");
	auto tex1 = new sf::Texture();
    if (!tex1->loadFromFile("assets/armatures/" + name + "/texture.png"))
    {
        LOG_ERROR("CAnnot!");
    }

	LOG_INFO("assets/armatures/" + name + "/atlas.json");
    auto tex = factory.loadTextureAtlasData("assets/armatures/" + name + "/atlas.json", tex1);
