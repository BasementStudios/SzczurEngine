...
public:
	...
	std::vector<std::vector<std::size_t>> 	verticesInTriagles;
...
public:
	...
	/* Methods */
public:
	void setMatrix(const Matrix& matrix, const glm::vec3& offset = {0.f, 0.f, 0.f}, const float& scale = 0.f) // @test 
	{
	...
	void setColor(const sf::Color& color)
	{
		for (std::size_t i = 0; i < this->verticesDisplay.size(); ++i) {
			this->verticesDisplay.setColor(i,  {color.r, color.g, color.b});
		}
	}
